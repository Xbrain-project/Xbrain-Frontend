...
function PostTeach() {                                                            
    ...
    const sentData = (teacher_id,values) => {
        createTeacherPost(teacher_id,values).then((res) => {
        } ).catch((err) => {
          //err
    ...
    const handleChange = (e) => {
      ...
      useEffect(() => {                                         //เรียกใช้เมื่อ detail มีการเปลี่ยนแปลง ไม่ใช้ useEffect ไม่ได้เพราะ setState ทำเสร็จเมื่อจบฟังก์ชั่น
        setPost({ ...postdata, detail: values.detail});
      },[values.detail]);
      ...
      const handleChangeSubjectCheckbox = (e) => {
        ...
        setSubject((subject)=> ({ ...subject, [e.target.name]: !subject[e.target.name] })); //คือการเรียกตัวแปรค่าเก่า ไม่ใช่อันแรกเริ่ม
      };
      useEffect(() => {
        ...
        setPost({ ...postdata, subject: newlistSubject });
      }, [subject, price]);
